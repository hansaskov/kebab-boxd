...
export async function validateSessionToken(token: string, db: DB) {
	const now = new Date();
...
export async function createSession(userId: number, db: DB): Promise<SessionWithToken> {
	const now = new Date();
...
export async function getSession(sessionId: string, db: DB) {
	const now = new Date();
